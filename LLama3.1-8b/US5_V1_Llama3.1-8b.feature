...
Scenario: Move task up button displayed for single selection
    ...
    And the "Arrow-down" button should be displayed next to the selected task
...
Scenario: Task moved up one position
    ...
    And the selected task has a sequence number of "<currentSequenceNumber>" in the "Tasks" table
    ...
    And the checkbox selection for the selected task remains
    And the "Arrow-down" button for the selected task should be enabled
