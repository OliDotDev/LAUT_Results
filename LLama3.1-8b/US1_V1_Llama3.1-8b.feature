...
Scenario: Add new aircraft button displayed
    ...
    And the modal contains input fields for "Name", "MSN", a "Manufacturer" dropdown, and a "Description" text area
    And the "Save" button and the "Cancel" button are displayed in the modal

Scenario: Modal for adding a new aircraft opened
    When the user clicks the "Add new aircraft" button
    Then the "Add new aircraft" modal should be displayed
    ...
    And the modal contains a "Description" text area
    And the modal contains a "Save" button
    And the modal contains a "Cancel" button

Scenario: Required fields in add aircraft modal
    When the user clicks the "Add new aircraft" button
    And the user enters a valid value into the "Name" input field
    And the user leaves the "MSN" input field empty
    And the user selects a manufacturer from the "Manufacturer" dropdown
    And the user enters a valid value into the "Description" text area
    And the user clicks the "Save" button
    Then the validation message "This field is required." should be displayed for the "MSN" input field
    When the user enters a valid value into the "MSN" input field
    And the user clicks the "Save" button
    Then a new aircraft should be added and listed in the table
...
Scenario: Modal closed without saving on cancel
    ...
    Then the modal should be closed
    And no new aircraft should have been added
