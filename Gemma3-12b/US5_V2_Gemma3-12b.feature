...
Scenario: Task moved up with "Arrow-up" button
    ...
    Then the task should move one position up in the "Tasks" table
...
Scenario: Task moved down with "Arrow-down" button
    ...
    And the checkbox selection should remain
    And the "Arrow-down" button should be disabled if the task is the last in the list

Scenario: Checkbox selection retained after moving task
    When the user selects the checkbox for the task in the "Tasks" table
    And the user clicks the "Arrow-up" button
    Then the checkbox for the task should remain selected
...
Scenario: Arrow-down button disabled for last task
    When the user selects the last task in the "Tasks" table
