...
Scenario: Document uploaded successfully
    When the user clicks the "Add new document" button
    And the user enters "<documentName>" into the "Name" input field
    And the user selects a PDF file via the "Document" file input
    And the user selects "<tag1>" and "<tag2>" from the "Tags" dropdown
    And the user clicks the "Save" button
    Then the "Add new document" modal should be closed
    And the new document with name "<documentName>" should be displayed in the list

Scenario: Document shown in list after upload
    When the user clicks the "Add new document" button
    And the user enters "<documentName>" into the "Name" input field
    And the user selects "<document>" from the "Document" file input
    And the user selects "<tags>" from the "Tags" dropdown
    And the user clicks the "Save" button
    Then the "Add new document" modal should be closed
    And the table should contain the value "<documentName>" in the "Name" column
...
Scenario: Tags dropdown populated
    When the user clicks the "Add new document" button
    Then the "Tags" dropdown should contain a list of all tags stored in the system
